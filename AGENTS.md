...
- First visit must not block on the entire 1,200-frame cinematic archive or sacrifice visual resolution. Gate entry on a contiguous high-resolution opening sequence, then complete the current chapter in the background and warm the next chapter in capped-concurrency windows. Keep scroll-time predecode work alive between scroll events; never cancel it merely because the target frame changes.
